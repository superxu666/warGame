namespace spine {
    ...
    export class SkeletonRenderer extends egret.DisplayObjectContainer {
        ...
        public clipper: SkeletonClipping = new SkeletonClipping();//GYLite 不能使用静态clipper，会被其他spine冲掉数据
        ...
        public constructor(skeletonData: SkeletonData) {
            ...
            for (let slot of this.skeleton.slots) {
                ...
                renderer.clipper = this.clipper;//GYLite
    ...
    export class SlotRenderer extends egret.DisplayObjectContainer {
        ...
        private attachmentColor:Color = new Color();
        private maskShape:egret.Shape;//GYLite
        public clipper:SkeletonClipping;//GYLite
        public constructor() {
            super();
        ...
        public renderSlot(slot: Slot, skeleton: Skeleton, colored: boolean) {
            ...
            let clipper = this.clipper;//SkeletonRenderer.clipper;
            ...
            // let attachmentColor = new Color()//GYLite 写成成员变量，不用每次都创建
            let attachmentColor = this.attachmentColor;
            this.attachmentColor.set(0,0,0,0);
            ...
            if (attachment instanceof RegionAttachment) {
                ...
                attachmentColor.setFromColor(attachment.color);//GYLite 使用成员变量拷贝
            ...
            if(drawFlag)
            {
                if (texture != null) {
                    ...
                    // if(finalIndicesLength == 186)
                    // {
                    //     Log.drawTriPath(npos,nindices);
                    // }
        ...
        private drawMesh(texture: egret.Texture, uvs, vertices, indices, color: Color) {
            ...
            if (color.r != 1 || color.g != 1 || color.b != 1 || color.a != 1) {
                if(color.r == 1 && color.g == 1 && color.b == 1 && color.a != 1)//GYLite 判断仅仅是alpha改变，我们不使用滤镜以节省性能
                {
                    meshObj.alpha = color.a;   
                    if(meshObj.filters)
                        meshObj.filters = null;
                }
                else
                {
                    var colorMatrix = [

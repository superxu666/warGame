class VerticalBox extends GYLite.GYScrollGroup {
	...
	public constructor(itemCls: any) {
		...
		s.scroller.verticalPolicy = 2;
		s.scrollerViewPort.canDrag = true;
		s.scrollerViewPort.horizonDrag= false;
	}
	...
	public validData(): void {
		...
		if (s._dataProider && s._dataProider.length > 0) {
			while (i < s._dataProider.length) {
				...
				s.addElement(item);
				s._lastPos = toY = item.y + item.height;
				++i;
			}
		}
	...
	public validPos(): void {
		...
		while (i < s.scrollerViewPort.numElement) {
			...
			gySp.y = toY;
			gySp.x = toX;
			s._lastPos = toY = gySp.y + gySp.height;
		}
	}
